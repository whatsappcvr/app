...
export default function ParentLoginScreen() {
  ...
  async function handleContinue(channel: 'sms' | 'admin') {
    const parsed = parseIdentifier(identifier)
    if (!parsed) {
      Alert.alert(
        'Invalid Details',
        "Please enter your ward's registered 10-digit roll number or your registered 10-digit mobile number.",
      )
      return
    }
    ...
    if (channel === 'admin') {
      router.push({
        pathname: '/(auth)/otp',
        params: { identifier: parsed.value, identifierType: parsed.type, accountType: 'parent' },
      })
      return
    }
    ...
    try {
      ...
      router.push({
        pathname: '/(auth)/otp',
        params: { identifier: parsed.value, identifierType: parsed.type, accountType: 'parent', channel },
      })
    } catch (error: any) {
      if (error.response?.status === 404) {
        Alert.alert('Not Registered', "This roll number / mobile number isn't registered as a parent contact. Please contact the college office.")
    ...
    <AppBackground style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <View style={styles.headerContainer}>
          <Text style={styles.title} onPress={handleTitleTap}>Parent Login</Text>
          <Text style={styles.subtitle}>Enter your ward's roll number or your registered mobile number</Text>
        </View>
        ...
        <TouchableOpacity
          ...
          onPress={() => handleContinue('sms')}
          disabled={sending}
        >
          <Ionicons name="chatbox-outline" size={18} color="#fff" style={styles.buttonIcon} />
          <Text style={styles.buttonText}>{sending ? 'Sending OTP...' : 'OTP via SMS'}</Text>
        </TouchableOpacity>
